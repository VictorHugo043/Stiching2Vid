...
- 当前没有完整实验驱动：
  - `scripts/legacy/ablate_seam.py`、`scripts/legacy/ablate_temporal.py` 存在，但当前只作为 legacy exploratory helpers
...
- 从本次文档更新开始：
  ...
  - 不再在 `ai-docs` 中标记为已实现
- 对已存在的 `scripts/legacy/ablate_seam.py`、`scripts/legacy/ablate_temporal.py`：
  - 保留为历史参考与便捷实验入口
